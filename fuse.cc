...
int myid;
yfs_client *yfs;
std::set<yfs_client::inum> inum_set;
...
yfs_client::inum
generate_inum(int isfile){
  //srand((unsigned int) time(NULL));
  yfs_client::inum result = rand();
  printf("generating.... inum : %016llx\n", result);
  if(isfile){
    result = result | (1<<31);
  }else{
    result = result & (~(1<<31));
  ...
  while(inum_set.find(result) != inum_set.end()){
    result = rand();
    if(isfile){
      result = result | (1<<31);
    }else{
                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    if( yfs->updatesize(ino, st.st_size) != yfs_client::OK){
      fuse_reply_err(req, ENOENT);
    }
#if 1
...
#if 1
  std::string buf;
  printf("what happened? here entered the fuseserver_read\n");
  if( yfs->read(ino,size,off,buf) != yfs_client::OK){
    printf("@@@@@@@@@@@entered here line:190 in fuse.cc");
    fuse_reply_err(req, ENOENT);
  }
  // Change the above "#if 0" to "#if 1", and your code goes here
  printf("we get the reply: *%s*\n",buf.c_str());
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  e->generation = 0;
  // You fill this in for Lab 2
  extent_protocol::attr result_attr;
  yfs_client::inum result_inum = generate_inum(1);
  printf("in fuse, the inum is %016llx\n",result_inum);
  if(yfs->create(parent,name,result_inum,result_attr) == yfs_client::OK){
    e->ino = result_inum;
    struct stat st;
    st.st_mode = S_IFREG | 0666;
    st.st_nlink = 1;
    st.st_atime = result_attr.atime;
    st.st_ctime = result_attr.ctime;
    st.st_mtime = result_attr.mtime;
    st.st_size = result_attr.size;
	e->attr = st;
	return yfs_client::OK;
  }
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  // You fill this in for Lab 2
  struct extent_protocol::attr result_attr;
  yfs_client::inum result_inum;
  if(yfs->lookup(parent,name,result_inum,result_attr,found) != yfs_client::OK){
    fuse_reply_err(req,ENOENT);  	
  }
  if (found){
    e.ino = result_inum;
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  if(yfs->readdir(ino,key_values) != yfs_client::OK){
    fuse_reply_err(req, ENOTDIR);
    return;
  }
  for(unsigned int i=1; i < key_values.size(); i += 2){
    const char *name = key_values[i].c_str();
    std::string the_inum = key_values[i-1];
	dirbuf_add(&b, name, yfs_client::n2i(the_inum));
  }
...
#if 1
  yfs_client::inum dir_inum = generate_inum(0);
  //extent_protocol::attr dir_attr;
  if(yfs->mymkdir(parent,name,dir_inum) != yfs_client::OK){
    fuse_reply_err(req,ENOENT);
  }
  e.ino = dir_inum;
