		...
		unsigned int xid_rep, char **b, int *sz)
{
	ScopedLock rwl(&reply_window_m_);
        ...
        unsigned int &min = window_bound_[clt_nonce];
          ...
          if(xid <= min){
            ...
            for(iter=reply_list.begin(); iter!=reply_list.end(); iter++){
              if(xid == iter->xid){
                ...
                //to check whether it is done or inprogress
                if(iter->cb_present){
                  ret = DONE;
                  *b = iter->buf;
                  *sz = iter->sz;
        ...
        if( ret == NEW){
          //insert the new reply_t into the list in an increasing order
          bool last = true;
          for(iter=reply_list.begin(); iter!=reply_list.end(); iter++){
            if(xid < iter->xid){
              last = false;
              reply_list.insert(iter,reply_t(xid));
              break;
            }
          }
          if(last == true){
          ...
          //since this is new, here should delete remembered requests
          //with XIDs <= xid_rep
          reply_t &head = reply_list.front();
          while(head.xid <= xid_rep){
            delete head.buf;
            reply_list.pop_front();
            head = reply_list.front();
          }
          min = xid_rep;  
        }
	return ret;
